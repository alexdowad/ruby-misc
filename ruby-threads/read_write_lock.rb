...
class ReadWriteLock
  def initialize
    ...
    @queue   = ConditionVariable.new # queue for waiting threads
    @mutex   = Mutex.new             # to protect queue
  end
  ...
  private
  def reader_running?
    (@counter.value & MAX_READERS) > 0
  end
  def too_many_readers?
    (@counter.value & MAX_READERS) == MAX_READERS
  end

  def writer_running?
    (@counter.value & RUNNING_WRITER) != 0
  end
  def writer_waiting?
    (@counter.value & MAX_WRITERS) != 0
  end
  def writer_present?
    @counter.value >= WAITING_WRITER
  end
  def too_many_writers?
    (@counter.value & MAX_WRITERS) == MAX_WRITERS
  end

  def thread_running?
    reader_running? || writer_running?
  end

  def sleep_if
    if slept = yield
      @mutex.synchronize do
        @queue.wait(@mutex) if slept = yield
      end
    end
    slept
  end
  ...
  def swap(old,new)
    @counter.compare_and_swap(old,new)
  end

  def acquire_read_lock
    with_counter do |c|
      raise "Too many reader threads!" if too_many_readers?
      if sleep_if { writer_present? }
        wake_one # so that all readers at front of queue will be woken up...
      else
        break if swap(c,c+1)
      end
    end
  ...
  def release_read_lock
    with_counter do |c|
      if swap(c,c-1)
        # If one or more writers are waiting, and we were the last reader, wake a writer up
        wake_one if writer_waiting? && ((c & MAX_READERS) == 1)
        break
      end
    end
  end
  ...
  def acquire_write_lock
    with_counter do |c|
      raise "Too many writers!" if too_many_writers?

      if not thread_running?
        break if swap(0,RUNNING_WRITER)
      elsif swap(c,c+WAITING_WRITER)
        with_counter do |c|
          unless sleep_if { thread_running? }
            break if swap(c,c+RUNNING_WRITER-WAITING_WRITER)
          end
        end
        break
      end
...
TOTAL_THREADS = 10 # set as high as practicable, for a better test
